...
public static class ProgramArena
{
    ...
    private static bool IsValidSelection(int indiceGuerrier1, int indiceGuerrier2)
    {
        return indiceGuerrier1 != indiceGuerrier2;
    }
    private static Arbitre SelectGuerriers()
    {
        ...
        do
        {
            CD.WriteAt($"Choisissez l'adversaire de {_guerriers[indiceGuerrier1].Nom}", 0, 0);
            CD.WriteAt("         ", 36, 13);
            indiceGuerrier2 = ReadValidInt("Entrer le numéro de l'adversaire 2 :", 13, 0, nbGuerrier - 1);
        } while (!IsValidSelection(indiceGuerrier1, indiceGuerrier2));
    ...
    public static void Main(string[] args)
    {
        ...
        do
        {
            ...
            int index = 0;
            while (!resultatTour.IsFinished() && tour <= 50)
            {
                Console.Clear();
            ...
            {
                Console.Clear();
                CD.WriteAt($"Le combat est terminé après {tour - 1} tours, il est déclaré nul !", 0, 0);
                Console.ReadLine();
            }
